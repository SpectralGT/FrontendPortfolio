...
const mouseBallBody = world.createRigidBody(
  RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(100, 100, 100)
);
const mouseBallShape = RAPIER.ColliderDesc.ball(3)
  .setMass(1)
  .setRestitution(15)
  .setFriction(100);
...
window.document.addEventListener("mousemove", (event) => {
  ...
  timeout = setTimeout(function () {
    mouseBallBody.setTranslation(new RAPIER.Vector3(100, 100, 100), false);
  }, 10);
...
function animate() {
  ...
  dynamicBodies.forEach((b) => {
    b[1].applyImpulse(
      new RAPIER.Vector3(
        -b[0].position.x * multiplier,
        -b[0].position.y * multiplier,
        -b[0].position.z * multiplier
      ),
      true
    );
  });
...
div2!.innerHTML = div2!.textContent!.replace(
  /\S/g,
  "<span class='letter' style='display: inline-block'>$&</span>"
);
